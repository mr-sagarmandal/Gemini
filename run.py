...
image = cv2.imread('aut.jpg', cv2.IMREAD_GRAYSCALE)#raw_input() need to change by imager = 50.0 / image.shape[1]
'''
dim = (50, int(image.shape[0] * r))
ss=int(image.shape[0])
mat=[[0 for x in range(50)]for y in range(int(image.shape[0]*r))]
...
'''
print (image)
mat=[[0 for x in range(image.shape[1])]for y in range(image.shape[0])]
...
while(rowline!=row):
    ...
    for i in range(FRow, row):
        if set == 1 and set2 == 0:
            set1 = 2
        for j in range(PLcol, col):
    ...
    while(scanline!=col):
        ...
        set,set1,set2,set3=0,0,0,0
        for i in range(Lcol,col):
            if set==1 and set2==0:
                set1=2
            for j in range(Frow,Lrow):
        ...
        if set3==0:
            Lcol=col
        set, set1, set2,set3 = 0, 0, 0,0
        for i in range(FRow,LRow+1):
            if set == 1 and set2 == 0:
                set1 = 2
            for j in range(PLcol,Lcol):
                if mat[i][j] == 0 and set == 0:
                    set = 1
                    Frow = i
                if mat[i][j] == 0 and set == 1:
